...
export default function AnalyticsPage() {
  ...
  const { data, status, loading, error, retry } = useAsyncData(
    () => getAnalytics(range),
    [range, transactions, budgets, goals],
  );

  const period = data ? `vs previous ${data.caption.replace("last ", "")}` : "";
  const hasActivity =
    data && (data.totals.income > 0 || data.totals.expenses > 0);
      ...
      {data && (
        ...
        >
          {status === "error" && (
            <div className="mb-4">
              <ErrorState
                title="Could not refresh analytics"
                message={error}
                onRetry={retry}
              />
            </div>
            ...
            <StatCard
              ...
              footer={
                data.totals.savingsRate >= 20
                  ? "At or above the 20% guideline"
                  : "Below the 20% guideline"
              }
            />
          </div>
          ...
          {!hasActivity ? (
            <div className="card mt-4">
              <EmptyState
                ...
                description={`Nothing was recorded in the ${data.caption}. Try a longer range.`}
              />
            </div>
          ) : (
            <>
              <div className="mt-4 grid gap-4 lg:grid-cols-3">
                <ChartCard
                  ...
                  description={`Expenses, ${data.caption}`}
                  updateKey={range}
                >
                  <SpendingAreaChart
                    data={data.buckets}
                    ...
                    name="Expenses"
                    height={270}
                    summary={`Expenses over the ${data.caption}`}
                  />
                </ChartCard>
                <ChartCard
                  title="Category breakdown"
                  description={`Share of spending, ${data.caption}`}
                  reveal="scale"
                  index={1}
                  updateKey={range}
                >
                  {data.categories.length ? (
                    ...
                    <EmptyState title="No expenses" className="py-10" />
                  )}
                </ChartCard>
              </div>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <ChartCard
                  ...
                  description={`Money received, ${data.caption}`}
                  updateKey={range}
                >
          ...
          <p className="mt-4 text-caption text-subtle">
            {data.health.goalProgress != null &&
              `Your goals are ${formatPercent(data.health.goalProgress, { digits: 0 })} complete on average. `}
            Pending and failed transactions are excluded from every figure on
            this page.
          </p>
        </div>
      )}
