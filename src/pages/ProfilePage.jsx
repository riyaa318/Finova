...
const pick = (user) =>
  Object.fromEntries(FIELDS.map((f) => [f, user[f] ?? ""]));
...
export default function ProfilePage() {
  ...
  const chooseAvatar = async (event) => {
    ...
    if (!file.type.startsWith("image/"))
      return toast.error("Choose an image file (JPG, PNG or WebP).");
    if (file.size > 5 * 1024 * 1024)
      return toast.error("That image is larger than 5 MB.");
    try {
  ...
  return (
    <div className="page-container">
      <PageHeader
        title="Profile"
        description="Your details and how you appear in FINOVA."
      />

      <div className="grid gap-4 lg:grid-cols-3">
        <aside
          ref={summaryRef}
          className="card h-fit p-6 text-center lg:text-left"
        ...
        >
          <div className="flex flex-col items-center gap-4 lg:items-start">
            ...
            <div>
              ...
              <p className="text-small text-muted">
                {values.occupation || "Add your occupation"}
              </p>
            </div>
            <input
              ref={fileRef}
          ...
          <ul className="mt-6 space-y-2.5 border-t border-line pt-5 text-left">
            ...
            <Fact icon={Calendar}>
              Member since {date.medium(user.createdAt)}
            </Fact>
          </ul>
        </aside>
        ...
        >
          ...
          <p className="mt-0.5 text-small text-muted">
            Changes apply across the app once you save.
          </p>
          {submitError && (
            <p
              role="alert"
              className="mt-4 rounded-control bg-danger/10 px-3 py-2.5 text-small font-medium text-danger"
            >
              {submitError}
            </p>
          )}
