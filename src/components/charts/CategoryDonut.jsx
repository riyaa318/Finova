...
export default function CategoryDonut({
  ...
  return (
    <div className="flex flex-col gap-5">
      <div
        className="relative mx-auto w-full max-w-[210px] shrink-0"
        style={{ height }}
        role="img"
        aria-label={`Spending by category, total ${money(total)}`}
      >
            ...
            >
              {items.map((item) => (
                <Cell
                  key={item.name}
                  fill={
                    item.name === "Other" ? chartColor(8) : colorOf(item.name)
                  }
                />
              ))}
            </Pie>
        ...
        >
          ...
          {focused && (
            <span className="text-caption tabular-nums text-muted">
              {formatPercent(focused.share, { digits: 0 })} of spending
            </span>
          )}
        </div>
      </div>
        ...
        {items.map((item, index) => (
          <li key={item.name}>
            ...
            >
              ...
              <span className="min-w-[4.5rem] text-right text-small font-semibold tabular-nums text-ink">
                {money(item.value)}
              </span>
            </button>
          </li>
