...
const POINTS = [
  {
    icon: PiggyBank,
    text: "Budgets that warn you before you overspend",
  },
  {
    icon: Target,
    text: "Goals with deadlines you can actually hit",
  },
  {
    icon: ChartNoAxesCombined,
    text: "Charts that explain your month in seconds",
  },
];

const LINE =
  "M0 150 C67 146 103 118 153 124 S250 96 303 84 S397 92 447 58 S550 26 600 12";
  ...
  useGSAP(
    () => {
      ...
      tl.from("[data-brand-line]", {
        ...
        .from(
          "[data-brand-card]",
          { opacity: 0, y: 24, scale: 0.96, duration: 0.7, ease: EASE.pop },
          0.9,
        );
    },
    { scope: rootRef },
    ...
    <div ref={rootRef} className="grid min-h-dvh lg:grid-cols-[1.05fr_1fr]">
      <aside className="relative hidden overflow-hidden bg-accent text-accent-fg lg:flex lg:flex-col lg:justify-between lg:p-12 xl:p-16">
        <div data-brand-line className="flex items-center gap-3">
          <Logo
            size={36}
            className="[&>rect]:fill-[rgb(var(--accent-fg))] [&>path]:stroke-[rgb(var(--accent))]"
          />
        ...
        <div className="relative">
          <h2
            data-brand-line
            className="max-w-md text-[2.75rem] font-extrabold leading-[1.05] tracking-tight"
          >
            Know where every rupee goes.
          </h2>
          <p data-brand-line className="mt-4 max-w-md text-body opacity-85">
            Track spending, hold budgets and reach savings goals from one calm
            dashboard.
          </p>
        ...
        <div className="relative h-56" aria-hidden="true">
          ...
          <div
            data-brand-card
            className="absolute right-0 top-0 rounded-card bg-surface px-5 py-4 text-ink shadow-pop"
          >
      ...
      <main className="flex items-center justify-center px-5 py-10 sm:px-10">
        <div ref={formRef} className="w-full max-w-md">
          ...
          <h1 className="text-h1 text-ink sm:text-[2rem] sm:leading-tight">
            {title}
          </h1>
