...
/** Slide-in navigation for tablet and mobile. Overlay fade -> panel slide -> item stagger (reversed on close). */
export default function MobileMenu({ open, onClose }) {
  const { user, logout } = useAuth();
  return (
    <Drawer open={open} onClose={onClose} label="Navigation menu">
      <div data-stagger className="flex h-16 shrink-0 items-center justify-between border-b border-line px-5">
      ...
      <div data-stagger className="shrink-0 space-y-1 border-t border-line p-3">
        <Link to="/profile" onClick={onClose} className="flex items-center gap-3 rounded-control p-2 transition-colors hover:bg-raised">
