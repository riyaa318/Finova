  ...
  return (
    <div className="space-y-4">
      ...
      >
        <form onSubmit={submit} noValidate className="grid max-w-md gap-4">
          {submitError && (
            <p
              role="alert"
              className="rounded-control bg-danger/10 px-3 py-2.5 text-small font-medium text-danger"
            >
              {submitError}
            </p>
          )}
      ...
      <SettingsSection
        ...
        description="Saved as preferences. This demo has no server, so nothing extra is enforced at sign-in."
      >
        <div className="space-y-5">
          ...
          {setting(
            "loginAlerts",
            "Sign-in alerts",
            "Get notified when your account is used on a new device.",
          )}
        </div>
      </SettingsSection>
    </div>
