...
export default function TransactionFilters({ filters, onChange, onClear }) {
  ...
  return (
    <section aria-label="Filter transactions" className="card mb-4 p-4">
      ...
      <div
        id="filter-panel"
        className={`mt-4 grid-cols-2 gap-3 md:grid-cols-3 lg:grid lg:grid-cols-6 ${panelOpen ? "grid" : "hidden"}`}
      >
        <FilterDropdown
          label="Category"
          ...
          options={asOptions(ALL_CATEGORIES)}
        />
        <FilterDropdown
          label="Type"
          allLabel="All types"
        ...
        <FilterDropdown
          ...
          options={asOptions(PAYMENT_METHODS)}
        />
        <FilterDropdown
          label="Sort by"
          value={filters.sort}
          ...
          <div className="col-span-2 grid grid-cols-2 gap-3 md:col-span-3 lg:col-span-6 lg:max-w-md">
            <label className="text-caption font-semibold text-muted">
              ...
              <input
                ...
                className="field-control mt-1 h-10 py-0"
              />
            </label>
            <label className="text-caption font-semibold text-muted">
              To
