...
export default function TransactionModal({
  ...
  const [values, setValues] = useState(() =>
    transaction
      ? {
          ...
          amount: String(toDisplay(transaction.amount)),
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...
      amount: toBase(Number(values.amount)),
    };
    try {
      ...
      setSubmitError(
        error.message ||
          "We could not save this transaction. Please try again.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Modal
      ...
      description={
        isEdit
          ? `Update ${transaction.merchant} (${transaction.id}).`
          : "Record income or an expense. Totals, budgets and charts update instantly."
      }
      footer={
        <>
