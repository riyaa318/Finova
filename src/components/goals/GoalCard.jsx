...
function statusBadge(goal) {
  if (goal.status === "achieved")
    return { tone: "success", icon: CircleCheck, text: "Achieved" };
  if (goal.status === "overdue")
    return { tone: "danger", icon: TriangleAlert, text: "Past deadline" };
    ...
    <article ref={ref} className="card flex flex-col p-5">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-small">
        ...
        <dd className="text-right tabular-nums text-ink">
          {date.medium(goal.deadline)}
        </dd>
      </dl>

      <div className="mt-4 flex flex-1 flex-col justify-end gap-2.5 border-t border-line pt-4">
        {goal.status === "active" && goal.suggestedMonthly > 0 && (
          <p className="text-caption text-muted">
            Save about {money(goal.suggestedMonthly)} a month to reach this on
            time.
          </p>
        )}
        <Button
          variant={goal.status === "achieved" ? "secondary" : "primary"}
          icon={goal.remaining === 0 ? undefined : Plus}
          fullWidth
