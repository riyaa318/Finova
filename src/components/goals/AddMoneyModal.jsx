...
export default function AddMoneyModal({ open, goal, onClose }) {
  ...
  const submit = async (event) => {
    ...
    if (base > goal.remaining + 0.005)
      return setError(
        `Only ${money(goal.remaining)} is needed to complete this goal.`,
      );
    if (base > available + 0.005)
      return setError(
        `Not enough available balance. You have ${money(available)} available.`,
      );
    setSubmitting(true);
    setError("");
    try {
  ...
  const chips = [...QUICK_AMOUNTS.filter((v) => v < cap), cap].filter(
    (v, i, list) => v > 0 && list.indexOf(v) === i,
  );
