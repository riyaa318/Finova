...
export function FieldMessage({ id, error, hint }) {
  if (error) {
    return (
      <p id={id} className="mt-1.5 flex items-start gap-1 text-caption font-medium text-danger">
...
export default function TextField({ label, error, hint, prefix, suffix, id, className = '', ...inputProps }) {
  const autoId = useId();
  ...
  return (
    <div className={className}>
      ...
      <div className="relative">
        {prefix && <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-small font-semibold text-muted">{prefix}</span>}
        <input
          id={inputId}
