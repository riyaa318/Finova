    ...
    <section ref={ref} className={`card p-5 ${className}`}>
      <header className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-small text-muted">Highest usage, last 30 days</p>
        </div>
        <Button as={Link} to="/budgets" variant="ghost" size="sm" iconRight={ArrowRight}>
          Manage
        </Button>
      </header>
      {top.length ? (
              ...
              <li key={b.id}>
                <div className="mb-1.5 flex items-center justify-between gap-2 text-small">
                  <span className="flex min-w-0 items-center gap-1.5 font-semibold text-ink">
                    <status.icon size={14} className={b.status === 'healthy' ? 'text-success' : b.status === 'warning' ? 'text-warning' : 'text-danger'} aria-label={status.label} />
                ...
                <ProgressBar value={b.percent} tone={status.bar} label={`${b.category} budget used`} size="h-1.5" />
              </li>
            );
          })}
        </ul>
      ) : (
        <EmptyState icon={PiggyBank} title="No budgets created" description="Set limits to see how spending compares." className="py-8" action={<Button as={Link} to="/budgets" state={{ openForm: true }}>Create a budget</Button>} />
      )}
    </section>
  );
}
