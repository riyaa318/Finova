...
/** Accessible dialog. Elements marked `data-stagger` inside `children` animate in one after another. */
export default function Modal({ open, onClose, title, description, size = 'md', children, footer }) {
  const titleId = useId();
  const descriptionId = useId();
  const { mounted, rootRef, overlayRef, panelRef } = useOverlayMotion({ open, variant: 'modal' });
  useDialogBehavior({ active: mounted && open, panelRef, onClose });

  if (!mounted) return null;
  return createPortal(
    <div ref={rootRef} className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-6">
      ...
      >
        <header data-stagger className="flex items-start justify-between gap-4 border-b border-line px-5 py-4">
          ...
          <IconButton label="Close dialog" icon={X} size="sm" onClick={onClose} />
        </header>
        <div className="scrollbar-thin flex-1 overflow-y-auto px-5 py-5">{children}</div>
        {footer && (
          <footer data-stagger className="flex flex-col-reverse gap-2 border-t border-line bg-raised/50 px-5 py-3.5 sm:flex-row sm:justify-end">
            {footer}
          </footer>
        )}
      </div>
