...
export default function Button({
  ...
  useButtonMotion(ref, { iconRef: IconRight ? undefined : iconRef, sheenRef: variant === 'primary' ? sheenRef : undefined });
  ...
  return (
    ...
    >
      {variant === 'primary' && (
        <span ref={sheenRef} aria-hidden="true" style={{ opacity: 0 }} className="pointer-events-none absolute inset-y-0 left-0 w-full bg-gradient-to-r from-transparent via-white/25 to-transparent" />
      )}
      {loading ? (
      ...
      {children}
      {IconRight && !loading && <IconRight size={16} strokeWidth={2.25} aria-hidden="true" />}
    </Component>
  );
}
