...
export default function Avatar({ name, src, size = 36, className = '' }) {
  ...
  return (
    <span aria-hidden="true" style={style} className={`inline-flex shrink-0 select-none items-center justify-center rounded-full bg-accent/15 font-bold text-accent ${className}`}>
      {initialsOf(name)}
    </span>
  );
}
