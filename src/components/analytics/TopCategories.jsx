  ...
  return (
    <section ref={ref} className={`card p-5 ${className}`}>
      ...
      <p className="mt-0.5 text-small text-muted">
        Where most of your money went, {caption}
      </p>
      {categories.length ? (
              ...
              <li key={c.name}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <ProgressBar
                      value={max ? (c.value / max) * 100 : 0}
                      label={`${c.name} share of top spending`}
                      size="h-1.5"
                      className="mt-1.5"
                    />
                  </div>
                ...
                <p className="ml-[46px] mt-1.5 flex items-center gap-2 text-caption text-muted">
                  <span className="tabular-nums">
                    {formatPercent(c.share, { digits: 0 })} of spending
                  </span>
                  {c.change != null && (rising || falling) ? (
                    ...
                    >
                      {formatPercent(c.change, { sign: true, digits: 0 })} vs
                      previous
                    </Badge>
                  ) : (
                    <span>
        ...
        <EmptyState
          ...
          description="Pick a longer range to see more."
          className="py-10"
        />
      )}
    </section>
