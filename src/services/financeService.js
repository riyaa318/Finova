import { ANALYTICS_RANGES, INCOME_CATEGORIES, EXPENSE_CATEGORIES } from '../utils/constants';
import { nowLocalISO, todayISO } from '../utils/dates';
import { computeAnalytics, computeBudgetUsage, computeDashboardStats, computeGoalProgress } from '../utils/finance';
...
export const getTransactions = (options) => respond(() => sortTransactions(db.transactions.read()), options);

export const getTransaction = (id, options) =>
  respond(() => db.transactions.read().find((t) => t.id === id) ?? fail('Transaction not found.', 404), options);
...
export const updateTransaction = (id, data, options) =>
  respond(() => {
    ...
    const next = { ...merged, merchant: merged.merchant.trim(), amount: round2(merged.amount), notes: (merged.notes ?? '').trim() };
    list[index] = next;
...
const assertBudget = ({ category, limit }, others) => {
  if (!EXPENSE_CATEGORIES.includes(category)) fail('Choose a valid expense category.');
  const problem = amountError(limit, { label: 'Budget limit' });
  if (problem) fail(problem);
  if (others.some((b) => b.category === category)) fail('A budget for this category already exists.', 409);
};

export const getBudgets = (options) =>
  respond(() => computeBudgetUsage(db.budgets.read(), db.transactions.read(), todayISO()), options);

export const createBudget = (data, options) =>
  respond(() => {
    const budgets = db.budgets.read();
    assertBudget(data, budgets);
    const budget = { id: makeId('bud'), category: data.category, limit: round2(data.limit) };
...
export const updateBudget = (id, data, options) =>
  respond(() => {
    ...
    assertBudget(merged, budgets.filter((b) => b.id !== id));
...
export const createGoal = (data, options) =>
  respond(() => {
    ...
    const goal = {
      ...
      currentAmount: round2(Math.min(Number(data.currentAmount) || 0, Number(data.targetAmount))),
...
export const updateGoal = (id, data, options) =>
  respond(() => {
    ...
    goals[index] = {
      ...
      currentAmount: round2(Math.min(Number(merged.currentAmount), Number(merged.targetAmount))),
    };
...
/** Moves money from the available balance into a goal. */
export const addMoneyToGoal = (id, amount, options) =>
  respond(() => {
    ...
    if (Number(amount) > remaining) fail(`Only ${inr(remaining)} is needed to complete this goal.`);
    const stats = computeDashboardStats({ transactions: db.transactions.read(), goals, openingBalance: db.account.read().openingBalance, today: todayISO() });
    if (Number(amount) > stats.availableBalance) fail(`Not enough available balance. You have ${inr(Math.max(0, stats.availableBalance))} available.`);
    goals[index] = { ...goal, currentAmount: round2(goal.currentAmount + Number(amount)) };
    db.goals.write(goals);
    return goals[index];
  }, options);
...
export const getAnalytics = (range, options) =>
  respond(() => {
    if (!ANALYTICS_RANGES.some((r) => r.value === range)) fail('Unknown time range.', 400);
...
export const resetDemoData = (options) => respond(() => (db.reset(), true), options);
export const clearAllData = (options) => respond(() => (db.clear(), true), options);

export const CATEGORY_LISTS = { income: INCOME_CATEGORIES, expense: EXPENSE_CATEGORIES };
