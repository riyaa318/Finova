...
export async function login({ email, password, remember = false }) {
  ...
  return respond(
    async () => {
      ...
      const valid =
        record && record.passwordHash === (await hashPassword(password));
...
export async function updateProfile(
  userId,
  { name, email, phone, occupation, location, avatar },
) {
  return respond(() => {
    const users = readUsers();
    ...
    if (users.some((u) => u.id !== userId && u.email === nextEmail))
...
export async function changePassword(userId, { currentPassword, newPassword }) {
  return respond(
    async () => {
      ...
      const index = users.findIndex((u) => u.id === userId);
      if (index === -1) throw new ApiError("Account not found.", 404);
      if (users[index].passwordHash !== (await hashPassword(currentPassword)))
