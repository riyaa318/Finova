...
export function matchesQuery(tx, query) {
  ...
  if (!q) return true;
  return [tx.merchant, tx.category, tx.notes, tx.id].some((field) =>
...
export const sortTransactions = (transactions, sort = "newest") => {
  ...
  switch (sort) {
    ...
    case "merchant":
      return list.sort(
        (a, b) => a.merchant.localeCompare(b.merchant) || byNewest(a, b),
      );
    default:
...
export const activeFilterCount = (filters) =>
  ["q", "category", "type", "method"].filter((key) => filters[key]).length +
