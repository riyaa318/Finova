  ...
  const money = (amount = 0) => {
    ...
    const isWhole =
      cfg.code === "INR"
        ...
        : Math.abs(value) >= 100;
    return (isWhole ? whole : precise).format(
      cfg.code === "INR" && isWhole ? Math.round(value) : value,
    );
  };

  const signed = (amount = 0) =>
    `${amount < 0 ? MINUS : "+"}${money(Math.abs(amount))}`;
...
export const chartColorAlpha = (index, alpha) =>
  `rgb(var(--chart-${index}) / ${alpha})`;
